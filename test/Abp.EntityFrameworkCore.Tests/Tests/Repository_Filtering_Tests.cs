...
namespace Abp.EntityFrameworkCore.Tests.Tests
{
    public class Repository_Filtering_Tests : EntityFrameworkCoreModuleTestBase
    {
        ...
        public async Task Should_Filter_MayHaveTenantId()
        {
            ...
            posts1.All(p => p.TenantId == GuidExtensions.Guid42).ShouldBeTrue();

            //Switch to host
            AbpSession.TenantId = null;
            
            var posts2 = await _postRepository.GetAllListAsync();
            posts2.Any(p => p.TenantId == GuidExtensions.Guid42).ShouldBeFalse();

            using (var uow = _unitOfWorkManager.Begin())
            {
                //Switch to tenant 42
                using (_unitOfWorkManager.Current.SetTenantId(GuidExtensions.Guid42))
                {
                    var posts3 = await _postRepository.GetAllListAsync(p => p.Title != null);
                    posts3.All(p => p.TenantId == GuidExtensions.Guid42).ShouldBeTrue();
                }

                var posts4 = await _postRepository.GetAllListAsync();
                posts4.Any(p => p.TenantId == GuidExtensions.Guid42).ShouldBeFalse();
                posts4.Any(p => p.TenantId == null).ShouldBeTrue();

                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    var posts5 = await _postRepository.GetAllListAsync();
                    posts5.Any(p => p.TenantId == GuidExtensions.Guid42).ShouldBeTrue();
        ...
        public async Task Should_Filter_MustHaveTenantId()
        {
            ...
            ticketsDefault.Any(t => t.TenantId == GuidExtensions.Guid1).ShouldBeTrue();
            ticketsDefault.Any(t => t.TenantId == GuidExtensions.Guid42).ShouldBeTrue();

            //Switch to tenant 42
            AbpSession.TenantId = GuidExtensions.Guid42;
            ticketsDefault = await _ticketRepository.GetAllListAsync();
            ticketsDefault.Any(t => t.TenantId == GuidExtensions.Guid42).ShouldBeTrue();
            ticketsDefault.Any(t => t.TenantId != GuidExtensions.Guid42).ShouldBeFalse();
